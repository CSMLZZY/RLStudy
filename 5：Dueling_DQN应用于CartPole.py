...
class Dueling_DQN():
    def __init__(self):
        ...
        # 其余超参数
        self.memory = deque(maxlen=2000)  # 经验重放队列，保存的数据最多为2000条
        self.batch = 128
        self.gamma = 0.95  # 折扣率
        self.learning_rate = 1e-3  # 学习率
        self.opt = tf.keras.optimizers.Adam(self.learning_rate)  # 优化器
        self.is_rend = False  # 默认不渲染，当达到一定次数后，开始渲染。

    """
    可以把dueling DQN分为三部分:
    第一部分：和普通DQN一样，用来处理和学习数据;
    第二部分：计算svalue(该state下的Q值的平均数);
    ...
    def get_model(self):
        ...
        mean = tl.layers.Lambda(lambda x: tf.reduce_mean(x, axis=1, keepdims=True))(avalue)  # 用Lambda层(自定义层)，计算avg(a)
        advantage = tl.layers.ElementwiseLambda(lambda x, y: x - y)([avalue, mean])  # 计算 a - avg(a)
        output = tl.layers.ElementwiseLambda(lambda x, y: x + y)([svalue, advantage])
        return tl.models.Model(inputs=input, outputs=output)

    def update_epsilon(self):
        # 更新epsilon：当小于epsilon_min时，不进行更新；否则比每次都乘以减少率epsilon_decay。
        if self.epsilon >= self.epsilon_min:
            self.epsilon *= self.epsilon_decay

    def update_target_Q(self):
        # Q网络学习完之后，需要把参数赋值到target_Q网络
        for i, target in zip(self.Q_network.weights, self.target_Q_network.weights):
            target.assign(i)
    ...
    def process_data(self):
        # 从经验回放队列中，随机取出一个batch大小的数据。
        data = random.sample(self.memory, self.batch)
        # s、s_需要转换为array，因为需要作为Q网络的输入
        ...
        for i, (_, a, _, r, done) in enumerate(data):
            if done:
                ...
                # 可以理解为：Q_network提议案，target_Q_network进行执行
    ...
    def update_Q_network(self):
        ...
        grads = tape.gradient(loss, self.Q_network.weights)
        self.opt.apply_gradients(zip(grads, self.Q_network.weights))
        return loss
    ...
    def train(self, episode):
        step = 0
        # 记录成绩大于200的游戏轮数（ep），因为分数大于200意味着游戏成功
        rend = 0
